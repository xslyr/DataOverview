...
class DataOverview:
    ...
    cols = ['dtype','count','mean','std','nullCount','min','max','mode','modeCount','25%','50%','75%']
    ...
    def columns_type(self):
        ...
        for i in df.columns:
            cols_dtype[ str(df[i].dtypes) ] +=1
        return  cols_dtype
    
    def show(self, type='all'):
        aux = pd.DataFrame([], index=self.cols)
        if type not in ['all']+self.possible_dtypes:
            raise ValueError('Error type parameter. Choose one of this values: {}'.format(['all']+possible_dtypes))
    ...
    def __extractData(self, i):
        ...
        aux.loc['nullCount'] = self.df[aux.columns[0] ].isnull().sum()
        aux.loc['mode'] = self.df[aux.columns[0]].mode(dropna=False).iloc[0]
        aux.loc['modeCount'] = self.df[aux.columns[0]].value_counts().sort_values(ascending=False).iloc[0]
